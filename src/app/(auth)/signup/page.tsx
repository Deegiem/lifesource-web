        ...
        <div className="mx-auto flex w-full max-w-2xl flex-1 flex-col pt-6 sm:pt-8">
          ...
          <form className="mt-7 space-y-4 md:mt-8 md:space-y-5" onSubmit={(e) => { e.preventDefault(); if (valid) router.push(AUTH_ROUTES.VERIFY); }}>
            {[
              ...
              <Field key={key} id={key} label={label} type={type} placeholder={placeholder}
                value={form[key as keyof typeof form]} onChange={(v) => update(key as keyof typeof form, v)} />
            ))}

            <PasswordField id="password" label="Password" value={form.password} show={show} toggle={() => setShow(!show)}
            ...
            {form.password && (
              <div className="space-y-2">
                {[
                  ...
                  <p key={label as string} className={`text-xs ${met ? "text-(--color-success)" : "text-(--color-text-muted)"}`}>
                    {met ? "✓" : "○"} {label as string}
                  </p>
                ))}
              </div>
            ...
            <PasswordField id="confirmPassword" label="Confirm password" value={form.confirmPassword} show={confirmShow}
              toggle={() => setConfirmShow(!confirmShow)} onChange={(v) => update("confirmPassword", v)} placeholder="Re-enter your password" />
            ...
            <label className="flex items-start gap-3 pt-1">
              ...
              <span className="text-xs leading-relaxed text-(--color-text-secondary)">
                I agree to the <span className="font-semibold text-(--color-brand-primary) underline">Terms of Service</span> and <span className="font-semibold text-(--color-brand-primary) underline">Privacy Policy</span>
              </span>
            </label>
...
function Field({ id, label, type, placeholder, value, onChange }: { id: string; label: string; type: string; placeholder: string; value: string; onChange: (v: string) => void }) {
  return <div className="space-y-2"><label htmlFor={id} className="text-sm font-semibold text-(--color-text-primary)">{label}</label>
    <input id={id} type={type} value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder}
      className="h-(--control-height-lg) w-full rounded-(--radius-lg) border border-(--color-border-default) px-4 text-sm text-(--color-text-primary) outline-none placeholder:text-(--color-text-muted) focus:border-(--color-brand-primary) focus:ring-2 focus:ring-(--color-brand-primary-soft)" required /></div>;
}

...
export default function ResetPasswordPage() {
  ...
  const valid = code.length === 6 && requirements.every(([, met]) => met) && password === confirm;
  ...
  return (
    <main className="min-h-screen bg-(--color-surface-card)">
      <div className="mx-auto flex min-h-screen w-full max-w-2xl flex-col px-5 py-6 sm:px-8 sm:py-8">
        <button type="button" onClick={() => router.push(AUTH_ROUTES.FORGOT_PASSWORD)} aria-label="Go back"
        ...
        <div className="flex-1 pt-6 sm:pt-8">
          <h1 className="text-2xl font-extrabold text-(--color-text-primary) sm:text-3xl">Create new password</h1>
          <p className="mt-1 text-sm text-(--color-text-secondary)">Enter the code sent to your email.</p>

          <div className="mt-7 space-y-2">
            <label htmlFor="code" className="text-sm font-semibold text-(--color-brand-primary)">Verification code</label>
          ...
          <PasswordField id="password" label="New password" value={password} show={show} toggle={() => setShow(!show)} onChange={setPassword}/>
          {password && <div className="mt-3 space-y-2">{requirements.map(([label, met]) =>
            <p key={label} className={`text-xs ${met ? "text-(--color-success)" : "text-(--color-text-muted)"}`}>{met ? "✓" : "○"} {label}</p>
          )}</div>}

          <div className="mt-5">
            <PasswordField id="confirm" label="Confirm new password" value={confirm} show={showConfirm} toggle={() => setShowConfirm(!showConfirm)} onChange={setConfirm} placeholder="Re-enter your password"/>
          </div>
        </div>
...
function PasswordField({ id, label, value, show, toggle, onChange, placeholder = "Enter your password" }: {
  id: string; label: string; value: string; show: boolean; toggle: () => void; onChange: (v: string) => void; placeholder?: string;
}) {
